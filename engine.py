...
async def process_chat(request: ChatRequest) -> ChatResponse:
    ...
    # 4. Cap at 12 records total to optimize recall while staying within reasonable token counts
    records = prioritized_records[:12]

    # ── Format catalog context ──────────────────────────────────────────────
    retrieved_str = format_retrieved_assessments(records)  # Include baseline/boosted/retrieved records fully up to cap of 12
